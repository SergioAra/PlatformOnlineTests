...
void FListViewWidget<DataType, DataViewWidget>::Update()
{
	if (bEnabled)
	{
		...
		for (size_t i = 0; i < DataWidgets.size(); ++i)
		{
			auto& Widget = DataWidgets[i];
			if (Widget)
			{
				if (FirstEntryToView + i < Data.size())
				{
					Widget->Show();
					Widget->SetData(Data[FirstEntryToView + i]);
				}
				else
				{
		...
		for (size_t i = 0; i < DataWidgets.size(); ++i)
		{
			...
			if (Widget->CheckCollision(Event.GetVector()))
			{
				...
				if (Widget && !Widget->IsFocused())
				{
					...
					{
						CallbackOnEntrySelected(FirstEntryToView + i);
					}
				}
...
void FListViewWidget<DataType, DataViewWidget>::RefreshData(const std::vector<DataType>& DataEntries)
{
	...
	if (Data.empty())
	{
		for (auto& NextWidget : DataWidgets)
		{
...
void FListViewWidget<DataType, DataViewWidget>::CreateListEntries()
{
	...
	for (size_t i = 0; i < NumEntriesOnScreen; ++i)
	{
		DataWidgets[i] = CreateListEntry<DataType, DataViewWidget>(Vector2(Position.x, Position.y + LabelHeight + (EntryHeight * i) + LabelVerticalOffset),
		...
		if (DataWidgets[i])
		{
			DataWidgets[i]->Create();
			DataWidgets[i]->Hide();
			DataWidgets[i]->SetFont(NormalFont);

			if (Scroller)
			{
				Vector2 EntrySize = Vector2(Size.x - Scroller->GetSize().x - 5.f, DataWidgets[i]->GetSize().y);
				DataWidgets[i]->SetSize(EntrySize);
			}
		}
	}
}
