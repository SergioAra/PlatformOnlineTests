...
void FLobbies::SubscribeToRTCEvents()
{
	...
	AddNotifyRTCRoomConnectionChangedOptions.LobbyId = CurrentLobby.Id.c_str();
	AddNotifyRTCRoomConnectionChangedOptions.LocalUserId = CurrentUserProductId;
...
void FLobbies::OnRTCRoomParticipantAudioUpdated(const char* RoomName, FProductUserId ParticipantId, bool bIsTalking, bool bIsAudioDisabled)
{
	// Ensure this update is for our room
	if (CurrentLobby.RTCRoomName.empty() || CurrentLobby.RTCRoomName != RoomName)
	{
	...
	for (FLobbyMember& LobbyMember : CurrentLobby.Members)
	{
		...
		// Only update the audio output status for other players (we control their own status)
...
void EOS_CALL FLobbies::OnRTCRoomParticipantAudioUpdateRecieved(const EOS_RTCAudio_ParticipantUpdatedCallbackInfo* Data)
{
	if (Data)
	{
		...
		FGame::Get().GetLobbies()->OnRTCRoomParticipantAudioUpdated(Data->RoomName, Data->ParticipantId, bIsTalking, bIsAudioDisabled);
	}
	else
	{
