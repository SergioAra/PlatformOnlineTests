...
void FCustomInviteReceivedDialog::SetPosition(Vector2 NewPosition)
{
	...
	AcceptInviteButton->SetPosition(Vector2(FromNameLabel->GetPosition().x, PayloadValue->GetPosition().y + PayloadValue->GetSize().y));
	DeclineInviteButton->SetPosition(Vector2(AcceptInviteButton->GetPosition().x + 200.f, AcceptInviteButton->GetPosition().y));
}
...
void FCustomInviteReceivedDialog::SetSize(Vector2 NewSize)
{
	...
	PayloadValue->SetSize(Vector2(300.f, 100.f));
