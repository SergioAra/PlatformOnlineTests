...
#define CUSTOMINVITE_DIALOG_SENDINVITEBUTTON_HEIGHT		50.f
...
void FCustomInvitesDialog::SetPosition(Vector2 Pos)
{
	...
	if (!WeakOuterMenu.expired())
	{
		...
		{
			CustomInviteSendDialog->SetPosition(Pos + Vector2((WindowSize.x - CustomInviteSendDialog->GetSize().x) / 2.0f, (WindowSize.y - CustomInviteSendDialog->GetSize().y) / 2.f));
		}
